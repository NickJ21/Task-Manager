...
def output_formatter(results):
    ...
    for result in results:                  # for each result in results
        res = {
            "id": result[0],                # mapping keys to values. ID is the key, result[0] is the value
            "name": result[1],              # Everything in the for loop is deleted when the loop exits
            "summary": result[2],
            "description": result[3],
            "is_done": result[4]
        }
        out.append(res)                     # Appends to the Out list 
    return out
